...
extends Buffer<K, V> implements InputCollector<K, V> {
	...
	public synchronized boolean init(DataInputStream ins, TaskID taskID, long length)
	throws IOException {
		Path initFileName = new Path("/tmp/iterativehadoop/initstatedata");
		LOG.info("init file name is " + initFileName);
		JInput input = new JInput(taskID, initFileName, length);

		// Copy data to local-disk
		OutputStream outs = null;
		int bytesRead = 0;
		try {
			outs = rfs.create(initFileName);

			byte[] buf = new byte[64 * 1024];
			int n = ins.read(buf, 0, (int) Math.min(buf.length, length));
			while (n > 0) {
				bytesRead += n;
				...
				reporter.progress();
				n = ins.read(buf, 0, (int) Math.min(buf.length, length - bytesRead));
			}

			LOG.info("Read " + bytesRead + " bytes from initialization");

			outs.close();
		} catch (IOException ioe) {
			LOG.info("Failed to initial from " + taskID, ioe);
		...
		if (bytesRead != length) {
			try {
				...
				LOG.info("Failed to discard initialization");
			} catch (Throwable t) {
			...
			throw new IOException("Incomplete initialization");
		}
