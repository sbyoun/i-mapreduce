...
public class TaskTracker 
  ...
  private SnapshotCompletionEvent snapshotEvent;
  ...
  synchronized void initialize() throws IOException {
    ...
    // RPC initialization
    int maxConnections = maxCurrentMapTasks + maxCurrentReduceTasks;
    //set the num handlers to max*2 since canCommit may wait for the duration
    //of a heartbeat RPC
    this.taskReportServer =
      RPC.getServer(this, bindAddress, tmpPort, 2 * maxConnections, false, this.fConf);
					  ...
					  if (task.isMapTask()) {
						  if (((MapTask)task).getPhase() == 
							  ...
							  if (rjob.getReduceFetchStatus() == null) {
								  ...
								  //LOG.info("I set the FetchStatus Class " + f);
					  ...
					  if (task.isMapTask() && task.isPipeline()) {
						  ...
						  if(rjob.jobConf.getBoolean("mapred.iterative.mapsync", false)){
							  ...
							  */
							  ...
							  if (rjob.getReduceFetchStatus() == null) {
								  //this is a new job; we start fetching its reduce events
								  //LOG.info("reduce task is " + reduceID);
								  if(reduceID != null){
									  f = new FetchStatus(reduceID.getJobID(), 1); 
									  rjob.setReduceFetchStatus(f);
								  }
							  }
							  //LOG.info("2reduce task is " + reduceID);
							  f = rjob.getReduceFetchStatus();
							  if(f != null) fList.add(f);	
        ...
        try {
            ...
            while (fReduceList.size() == 0 && fMapList.size() == 0) {
              try {
                ...
                return;
            	  ...
            	  /*
            	  LOG.info("it is here "  + e.getMessage() + 
            			  " fReduceList size: " + fReduceList.size() +
            			  " fMapList size: " + fMapList.size());
            			  */
            	  Thread.sleep(1000);
            	  //fReduceList.clear();
            	  //fMapList.clear();
          ...
          for (FetchStatus f : fMapList) {
            ...
            try {
              //the method below will return true when we have not 
              //fetched all available events yet
            	//LOG.info("map fetchstatus " + f.fromEventId + " start");
          ...
          for (FetchStatus f : fReduceList) {
              ...
              try {
                //the method below will return true when we have not 
                //fetched all available events yet
            	  //LOG.info("reduce fetchstatus " + f.fromEventId + " start");
                if (f.fetchCompletionEvents(currentTime, false)) {
                  fetchAgain = true;
                }
              } catch (Exception e) {
   ...
   *  we commit it to the snapshot event to job tracket, let jobtracket do the 
   *  combination work
 * @throws Exception 
   */
	@Override
