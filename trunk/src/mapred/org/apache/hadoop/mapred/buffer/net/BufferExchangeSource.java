...
public abstract class BufferExchangeSource<H extends OutputFile.Header> 
	...
	public static final BufferExchangeSource factory(FileSystem rfs, JobConf conf, BufferRequest request) {
		...
		if (request.bufferType() == BufferType.ITERATE) {
			return new IterateSource(rfs, conf, request);
		}
	...
	*/
	...
	private static class IterateSource extends BufferExchangeSource<OutputFile.IterateHeader> {
		
		private Map<TaskID, Long> cursor;
		
		public IterateSource(FileSystem rfs, JobConf conf, BufferRequest request) {
			super(rfs, conf, request);
			this.cursor = new HashMap<TaskID, Long>();
		}
		...
		@Override
		protected final Transfer transfer(OutputFile file) {
			OutputFile.IterateHeader header = (OutputFile.IterateHeader) file.header();
			TaskID taskid = header.owner().getTaskID();
			...
			if (!cursor.containsKey(taskid) || cursor.get(taskid) == header.iteration()) { 
				BufferExchange.Connect result = open(BufferType.ITERATE);
				if (result == Connect.OPEN) {
