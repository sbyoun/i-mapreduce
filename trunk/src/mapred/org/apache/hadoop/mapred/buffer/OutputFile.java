...
public class OutputFile implements Comparable<OutputFile>, Writable {
	public static enum Type {FILE, SNAPSHOT, STREAM, ITERATE};
	public int STOP = -100;
	public int RUN = -200;
	...
	public abstract static class Header implements Writable, Comparable<Header> {
		...
		public static Header readHeader(DataInput in) throws IOException {
			...
			case ITERATE: header = new IterateHeader(); break;
			default: return null;
			}
	...
	public static class IterateHeader extends Header {
		
		private int iteration;
		
		public IterateHeader() { super(Type.ITERATE, null, 0f, false); }
		
		public IterateHeader(TaskAttemptID owner, int iteration) {
			super(Type.ITERATE, owner, 0f, false);
		...
		public int compareTo(Header header) {
			if (header instanceof IterateHeader) {
				IterateHeader other = (IterateHeader) header;
				if (owner.equals(other.owner)) {
					if (iteration < other.iteration) {
						return -1;
					}
					else if (iteration > other.iteration) {
						return 1;
					}
					else return 0;
				}
		...
		@Override
		public boolean equals(Object o) {
			if (o instanceof IterateHeader) {
				return this.compareTo((IterateHeader) o) == 0;
			}
			return false;
		}
		...
		@Override
		public int hashCode() {
			return Integer.toString(this.iteration).hashCode();
		}
		...
		@Override
		public String toString() {
			return super.toString() + " iteration: " + this.iteration;
		}
	}
	...
	public OutputFile(TaskAttemptID owner, int iteration, Path data, Path index, int partitions) {
		this.type = Type.ITERATE;
		this.data = data;
		this.index = index;
		this.partitions = partitions;					
		this.header = new IterateHeader(owner, iteration);
	}
	...
	public void delete(FileSystem fs) throws IOException {
		if (this.data != null && fs.exists(this.data)) {
			//System.out.println("delete file " + this.data);
