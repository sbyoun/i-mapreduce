...
public class JOutputBuffer<K extends Object, V extends Object> 
	...
	private class PartitionBufferMerger {
		...
		public synchronized OutputFile mergeIterate(int iteration) throws IOException {
			List<PartitionBufferFile> mergeSpills = new ArrayList<PartitionBufferFile>();
			long dataSize = 0;
			long indexSize = partitions * MAP_OUTPUT_INDEX_RECORD_LENGTH;
			...
			merge(mergeSpills, snapshot);
			LOG.info("iterate file size " + snapshot.dataSize());
			return new OutputFile(taskid, iteration, snapshot.data, snapshot.index, partitions);
		}
	...
	private class SpillThread extends Thread {
		...
		public boolean forceSpill(int index) throws IOException {
			synchronized (spillLock) {
				kvend = kvindex;
				bufend = bufmark;
				if (kvstart != kvend) {
					spill(index); // force a spill
					return true;
				} else {
		...
		public void close() throws IOException {
			...
			synchronized (spillLock) {
				...
				boolean spill = forceSpill(-1); // flush what remains
				if (pipeline) {
					if (!spill) {
		...
		private void spill(int index) throws IOException {
			if (kvstart != kvend) {
				float reduction = sortAndSpill(index);
						...
						try {
							if(iterative){
								synchronized(spillLock){
									spill(-1);
									/*
									forceSpill(iteration);
									LOG.info("output too large, spill automatically, now iterate " + iteration);
									OutputFile stream = merger.mergeStream(iteration);
									iteration++;
									
									if (stream != null ) {
										umbilical.output(stream);
										LOG.info("output this spill file, and next iteration is " + iteration);
									}
	...
	public JOutputBuffer(BufferUmbilicalProtocol umbilical, Task task, JobConf job, 
		...
		LOG.info("max memory usage is " + maxMemUsage);
		
		int recordCapacity = (int)(maxMemUsage * recper);
		recordCapacity -= recordCapacity % RECSIZE;
		kvbufferSize = maxMemUsage - recordCapacity;
		LOG.info("max buffer size is " + kvbufferSize);
		kvbuffer = new byte[(int)kvbufferSize];
		bufvoid = kvbuffer.length;
		recordCapacity /= RECSIZE;
		LOG.info("record capacity is " + recordCapacity);
		kvoffsets = new int[recordCapacity];
		kvindices = new int[recordCapacity * ACCTSIZE];
		softBufferLimit = (int)(kvbuffer.length * spillper);
		softRecordLimit = (int)(kvoffsets.length * spillper);
		LOG.info("softBufferLimit is " + softBufferLimit);
		LOG.info("softRecordLimit is " + softRecordLimit);
		// k/v serialization
	...
	public synchronized void stream(long sequence, boolean reset) throws IOException {
		LOG.info("JBuffer " + taskid + " performing stream snapshot. sequence " + sequence);
		spillThread.forceSpill(-1);
	...
	//new added method, should try
	public synchronized void iterate(int iteration) throws IOException {
		synchronized(spillLock){
			//spillThread.forceSpill(iteration);
			spillThread.forceSpill(-1);
			OutputFile iteratefile = merger.mergeIterate(iteration);
			
			if (iteratefile != null ) {
				umbilical.output(iteratefile);
			}
		}
	...
	public synchronized void clear() {	
		...
		kvbuffer = null;
		kvbuffer = new byte[this.iterative_spill_bufsize];
	}
	...
	public class Buffer extends OutputStream {
		...
		throws IOException {
			...
			try{
				...
				e.printStackTrace();
				LOG.info(" off: " + off + " bufindex: " + bufindex + " len : " + len);
			}catch(NullPointerException e2){
				e2.printStackTrace();
				LOG.error("b: " + b + " off: " + off + " bufindex: " + bufindex + " kvbuffer: " + kvbuffer + " len : " + len);
				LOG.info("b: " + b + " off: " + off + " bufindex: " + bufindex + " kvbuffer: " + kvbuffer + " len : " + len);
			}	
	...
	private float sortAndSpill(int index) throws IOException {
		...
		synchronized (mergeLock) {
			...
			try {
				// create spill file, I add a index parameter
				Path filename = null;
				if(index == -1){
					filename = outputHandle.getSpillFileForWrite(this.taskid, spills.size(), filesize);
				}else{
					filename = outputHandle.getSpillFileForWrite(this.taskid, index, filesize);
				}
				...
				if(index == -1){
					...
					indexFilename = outputHandle.getSpillIndexFileForWrite(
							this.taskid, index, partitions * MAP_OUTPUT_INDEX_RECORD_LENGTH);
				}
				...
				PartitionBufferFile spill = null;
				if(index == -1){
					spill = new PartitionBufferFile(spills.size(), filename, indexFilename, progress.get(), this.eof);
				}else{
					spill = new PartitionBufferFile(index, filename, indexFilename, progress.get(), this.eof);
				}
				
				if(index == -1){
					LOG.info("Finished spill " + spills.size());
				}else{
					LOG.info("Finished spill " + index);
				}
